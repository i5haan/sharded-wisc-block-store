...
class HafsClientShardFactory {
    ...
    vector<HafsClientFactory> temp_shard_list;
            ...
            if (shards.size() == 0) {
                ...
                for(int i = 0; i < shards.size(); i++) {
                    if (shard_servers.find(shards[i].id()) == shard_servers.end()) {
                        ...
                        temp_shard_list.push_back(HafsClientFactory(shards[i].primaryaddr(), shards[i].backupaddr()));
...
*/
        ...
        bool TriggerShuffle()
        {
            unordered_map<int, HafsClientFactory> ::iterator it;
            bool flag = true;
            int  numShards = temp_shard_list.size();
            for(int i=0;i<numShards;i++)
            {
                flag = flag & temp_shard_list[i].TriggerShuffle(numShards);
            }
            return flag;
        }
